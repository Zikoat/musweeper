...
#	optimizer = optim.Adam(model.parameters(), lr=3e-4, weight_decay=0.01)
	...
	while not timeout() and i < 1000:
		...
		last_game = play_game(model, env, custom_end_function=lambda env: env.unnecessary_steps > 0 or np.count_nonzero(np.logical_and(env.open_cells, env.mines)) > 0, custom_reward_function=lambda env, done: (1 if not done else -1))
		...
		i += 1
	sample_game = play_game(model, env, custom_end_function=lambda env: env.unnecessary_steps > 0 or np.count_nonzero(np.logical_and(env.open_cells, env.mines)) > 0, custom_reward_function=lambda env, done: (1 if not done else -1))
	for event in sample_game.history:
		report.add_note('state : {state}, action: {action}, reward: {reward}'.format(state=event.state, action=event.action, reward=event.reward))
...
#history = play_game(model, env, custom_end_function=lambda env: env.unnecessary_steps > 0 or np.count_nonzero(np.logical_and(env.open_cells, env.mines)) > 0, custom_reward_function=lambda env, done: (1 if not done else -1))
