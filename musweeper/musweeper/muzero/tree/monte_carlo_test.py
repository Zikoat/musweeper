...
class TestMonteCarlo(unittest.TestCase):
    def test_should_expand_tree(self):
        ...
        while current_node is not None:
            assert (current_node.children == {}
                    or len(current_node.children) == 1)
            current_node = None if len(current_node.children.keys(
            )) == 0 else current_node.children[list(current_node.children.keys())[0]]
            depth_counter += 1
    ...
    def test_should_update_correctly(self):
        ...
        values = [child_nodes.upper_confidence_boundary()
                  for child_nodes in tree.root.children.values()]
        assert 1 == len(values)
        assert 0 == values[0]
        assert 0 == tree.root.upper_confidence_boundary()
    ...
    def test_monte_carlo_backpropgate(self):
        ...
        assert 0 < tree.backpropgate(
            final_expanded_node, depth=max_search_depth, discount=1)
    ...
    def test_that_node_values_propgate_correctly_with_model(self):
        ...
        for best_action in range(2):
            ...
            model = muzero(env, representation, dynamics,
                           prediction, max_search_depth)
            output = model.plan_action(env.reset())
            assert len(output) == 2
            assert max(output, key=lambda x: x.score_metric()
                       ).node_id == best_action
