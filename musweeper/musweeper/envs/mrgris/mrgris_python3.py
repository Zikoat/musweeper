...
def parse_api_payload(payload):
	if 'board' in payload:
		...
		rules = [Rule(r['num_mines'], r['cells']) for r in payload['rules']]
		try:
			mine_p = payload['mine_prob']
		except KeyError:
			mine_p = MineCount(payload['total_cells'], payload['total_mines'])
...
def api_solve(payload):
	...
	start = time.time()
	try:
		result['solution'] = solve(rules, mine_p, '_other')
...
def generate_rules(board, total_mines, everything_mode=False):
	...
	def _rule(mines, cells):
		...
		if mines or cells:
			yield Rule(mines, [cell.name for cell in cells])
